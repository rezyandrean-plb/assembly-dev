...
import { InstructorCard, type InstructorProps } from "@/app/components/instructor-card"
import {
  Clock,
  Users,
  BarChart,
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (timeDelta > 0) {
        ...
        const relatedCoursesPosition = relatedCoursesRef.current?.getBoundingClientRect().top
        const hasReachedRelatedCourses = relatedCoursesPosition !== undefined && relatedCoursesPosition <= 100

        // Check if main content section bottom is in view
        const mainContentPosition = mainContentRef.current?.getBoundingClientRect()
        const hasReachedMainContentBottom =
          mainContentPosition !== undefined && mainContentPosition.bottom <= window.innerHeight

        if (!hasReachedRelatedCourses && !hasReachedMainContentBottom) {
          // Calculate new position (with limits to keep it visible)
          const newPosition = Math.max(
            24,
            Math.min(currentScrollY + 24, document.body.scrollHeight - windowHeight - 400),
          )
  ...
  return (
    <NetworkProvider>
      <main className="relative overflow-hidden bg-[#F5F5F5] min-h-screen">
        <NetworkBackground scrollY={scrollY} scrollSpeed={scrollSpeed} windowHeight={windowHeight} opacity={0.3} />
        <Navbar />
        {/* Hero Section */}
        <section className="pt-32 pb-16 relative z-10">
          <div className="container mx-auto px-4 relative z-10">
            <motion.div
              ...
              className="mb-6"
            >
              <a href="/courses" className="inline-flex items-center text-[#123B79] hover:underline">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10">
              ...
              >
                <h1 className="text-4xl font-bold text-[#123B79] mb-4 drop-shadow-sm">{courseData.title}</h1>

                <div className="flex flex-wrap items-center gap-4 mb-6">
                  <div className="flex items-center">
                    <Star className="h-5 w-5 text-[#F0A500] fill-[#F0A500]" />
                    <span className="ml-1 font-semibold">{courseData.rating}</span>
                    <span className="ml-1 text-gray-500">({courseData.students} students)</span>
                  </div>
                ...
                <div className="mb-8">
                  <h2 className="font-semibold text-xl text-[#123B79] mb-4">Course Instructors</h2>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                      {Array.isArray(courseData.instructors) && courseData.instructors.length > 0 ? (
                        courseData.instructors.map((instructor, index) => (
                          <InstructorCard key={index} name={instructor.name} image={instructor.image} />
                        ))
                      ) : (
                        <div className="text-gray-500 col-span-4">Instructor information coming soon.</div>
                      )}
                    </div>
                  </div>
                </div>
                ...
                {courseData.highlights && (
                  <div className="bg-white rounded-xl p-6 mb-8 shadow-md border border-gray-100">
                    <h2 className="text-xl font-bold text-[#123B79] mb-4">Course Highlights</h2>
                ...
                {/* What You'll Learn */}
                <div className="bg-white rounded-xl p-6 mb-8 shadow-md border border-gray-100">
                  <h2 className="text-xl font-bold text-[#123B79] mb-4">What You'll Learn</h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {Array.isArray(courseData.whatYouWillLearn) && courseData.whatYouWillLearn.length > 0 ? (
                      ...
                      <div className="text-gray-500">Learning outcomes will be updated soon.</div>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-white rounded-xl p-6 mb-8 shadow-md border border-gray-100">
                  <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-bold text-[#123B79]">Course Content</h2>
                  ...
                  <div className="space-y-4">
                    {Array.isArray(courseData.curriculum) && courseData.curriculum.length > 0 ? (
                          ...
                          {expandedModules.includes(moduleIndex) && (
                            <div className="divide-y divide-gray-200">
                              {module.lessons.map((lesson, lessonIndex) => (
                                <div key={lessonIndex} className="px-6 py-4 flex items-center">
                                  <div className="flex items-center">
                      ...
                      <div className="text-gray-500">No course content available.</div>
                    )}
                  </div>
                </div>
                ...
                {courseData.targetAudience && (
                  <div className="bg-white rounded-xl p-6 mb-8 shadow-md border border-gray-100">
                    <h2 className="text-xl font-bold text-[#123B79] mb-4">Who This Course Is For</h2>
                ...
                <div className="bg-white rounded-xl p-6 mb-8 shadow-md border border-gray-100">
                  <h2 ref={reviewsSectionRef} className="text-2xl font-bold text-[#123B79] mb-8">
                  ...
                  {hasReviews ? (
                              ...
                              <div className="flex items-center mt-1">
                                {[...Array(5)].map((_, i) => (
                                  <Star
                                    key={i}
                                    className={`h-4 w-4 ${
                                      i < review.rating ? "text-[#F0A500] fill-[#F0A500]" : "text-gray-300"
                                    }`}
                                  />
                                ))}
                    ...
                    >
                      ...
                      <Button onClick={() => {}} variant="outline" className="text-[#123B79] border-[#123B79]">
                        Write a Review
                      </Button>
                    </motion.div>
                  )}
              ...
              >
                ...
                >
                  ...
                  {/* Enrollment Card */}
                  <div className="bg-white/95 backdrop-blur-sm rounded-xl overflow-hidden shadow-lg border border-gray-200 mb-6">
                    <div className="p-6">
                      <div className="flex justify-between items-center mb-4">
                        <div className="text-3xl font-bold text-[#123B79]">{courseData.price}</div>
                      ...
                      <div className="space-y-4 mt-6">
                        ...
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">{courseData.students}</p>
                          </div>
                        </div>
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">{courseData.level}</p>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        {/* Related Courses */}
        <section ref={relatedCoursesRef} className="py-16 bg-gray-50 relative z-10 border-y border-gray-100">
          <div className="container mx-auto px-4">
            <h2 className="text-2xl font-bold text-[#123B79] mb-8">Related Courses You Might Like</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                ...
                >
                  ...
                  <div className="p-6">
                    <h3 className="text-lg font-bold text-[#123B79] mb-2">{relatedCourse.title}</h3>
                    <div className="text-gray-500 text-sm mb-4">
                      <span className="inline-block mr-4">⏱️ {relatedCourse.duration}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="font-bold text-[#123B79]">{relatedCourse.price}</span>
                      <Button variant="outline" size="sm" onClick={() => router.push(`/courses/${relatedCourse.slug}`)}>
                        View Course
                      </Button>
                    </div>
                  </div>
        ...
        {/* Final CTA */}
        <section className="py-16 bg-[#123B79] text-white relative z-10 border-y border-gray-100">
          <div className="container mx-auto px-4 text-center">
            <h2 className="text-3xl font-bold mb-4">Ready to advance your property investment knowledge?</h2>
            <p className="text-xl mb-8 max-w-2xl mx-auto">
              Join {courseData.students}+ students who are already transforming their investment strategies with this
              course.
            </p>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl overflow-hidden max-w-md w-full">
              <div className="flex justify-between items-center p-4 border-b">
                ...
                <button onClick={() => setIsShareModalOpen(false)} className="p-1 rounded-full hover:bg-gray-100">
                  <X className="h-5 w-5" />
                </button>
              </div>
              <div className="p-6">
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl overflow-hidden max-w-md w-full">
              <div className="flex justify-between items-center p-4 border-b">
                ...
                <button onClick={() => setIsLoginModalOpen(false)} className="p-1 rounded-full hover:bg-gray-100">
                  <X className="h-5 w-5" />
                </button>
              </div>
              <div className="p-6">
                <p className="mb-6">Please log in to add this course to your wishlist.</p>
                <div className="space-y-4">
                  <div>
                  ...
                  <Button onClick={handleLogin} className="w-full bg-[#123B79] hover:bg-[#0A2A5E]">
                    Log In
                  </Button>
                  <div className="text-center text-sm text-gray-500">
                    <a href="/forgot-password" className="text-[#123B79] hover:underline">
                      Forgot password?
                    </a>
