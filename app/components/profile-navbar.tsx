"use client";

import { useState, useEffect } from "react";
import Link from "next/link";
import Image from "next/image";
import { useRouter } from "next/navigation";
import {
  Bell,
  ChevronDown,
  User,
  LogOut,
  Settings,
  FileText,
  ShoppingBag,
} from "lucide-react";
import CartIcon from "./cart-icon";

export default function ProfileNavbar() {
  const router = useRouter();
  const [isScrolled, setIsScrolled] = useState(false);
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  const [hasNotifications, setHasNotifications] = useState(true);
  const [isNotificationsOpen, setIsNotificationsOpen] = useState(false);

  useEffect(() => {
    const handleScroll = () => {
      setIsScrolled(window.scrollY > 10);
    };

    window.addEventListener("scroll", handleScroll);
    return () => {
      window.removeEventListener("scroll", handleScroll);
    };
  }, []);

  const toggleDropdown = () => {
    setIsDropdownOpen(!isDropdownOpen);
    if (isNotificationsOpen) setIsNotificationsOpen(false);
  };

  const toggleNotifications = () => {
    setIsNotificationsOpen(!isNotificationsOpen);
    if (isDropdownOpen) setIsDropdownOpen(false);
  };

  const handleNavigation = (href) => {
    setIsDropdownOpen(false);
    setIsNotificationsOpen(false);
    router.push(href);
  };

  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled ? "bg-white shadow-md py-2" : "bg-white shadow-sm py-3"
      }`}
      data-oid="-fsezk-"
    >
      <div
        className="container mx-auto px-4 flex justify-between items-center"
        data-oid="g-e227k"
      >
        <Link href="/" className="flex items-center" data-oid="3t6w5u9">
          <Image
            src="/public/images/assembly-logo.png"
            alt="Assembly Logo"
            width={180}
            height={50}
            className="object-contain"
            data-oid="wcokcjf"
          />
        </Link>

        <div
          className="hidden md:flex items-center space-x-8"
          data-oid="7trl0pv"
        >
          <Link href="/courses" className="nav-link" data-oid="h9--zi6">
            Courses
          </Link>
          <Link href="/about" className="nav-link" data-oid="lfqmlhz">
            About
          </Link>
          <Link href="/plb-book" className="nav-link" data-oid=":gz47_k">
            PLB Book
          </Link>
          <Link href="/contact" className="nav-link" data-oid="cz.7e86">
            Contact
          </Link>

          <div className="flex items-center space-x-4" data-oid="tsjydwf">
            <CartIcon data-oid="k8r11b4" />

            <div className="relative" data-oid="tijwacu">
              <button
                onClick={toggleNotifications}
                className="relative p-1 rounded-full hover:bg-gray-100"
                data-oid="frlb24."
              >
                <Bell className="h-6 w-6 text-gray-600" data-oid="phe:eh8" />
                {hasNotifications && (
                  <span
                    className="absolute top-0 right-0 block h-2.5 w-2.5 rounded-full bg-red-500 ring-2 ring-white"
                    data-oid="hfcjv_5"
                  ></span>
                )}
              </button>

              {isNotificationsOpen && (
                <div
                  className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-lg py-2 z-50 border border-gray-100"
                  data-oid=":nu54u3"
                >
                  <div
                    className="px-4 py-2 border-b border-gray-100"
                    data-oid="1.i0a-2"
                  >
                    <div
                      className="flex justify-between items-center"
                      data-oid="-nc3k0c"
                    >
                      <h3
                        className="font-semibold text-gray-800"
                        data-oid="tha5669"
                      >
                        Notifications
                      </h3>
                      <button
                        className="text-xs text-blue-600 hover:underline"
                        data-oid="4-b6uzq"
                      >
                        Mark all as read
                      </button>
                    </div>
                  </div>
                  <div className="max-h-80 overflow-y-auto" data-oid="5xyqu7b">
                    <div
                      className="px-4 py-3 border-b border-gray-100 bg-blue-50"
                      data-oid="ym7tgv:"
                    >
                      <p
                        className="text-sm font-medium text-gray-800"
                        data-oid="uzz8o6g"
                      >
                        New course available
                      </p>
                      <p
                        className="text-xs text-gray-500 mt-1"
                        data-oid="asicn8m"
                      >
                        "Advanced Property Investment Strategies" is now
                        available.
                      </p>
                      <p
                        className="text-xs text-gray-400 mt-1"
                        data-oid="_r.69pn"
                      >
                        2 hours ago
                      </p>
                    </div>
                    <div
                      className="px-4 py-3 border-b border-gray-100"
                      data-oid="g6wa8u."
                    >
                      <p
                        className="text-sm font-medium text-gray-800"
                        data-oid="6erv0x3"
                      >
                        Course completed
                      </p>
                      <p
                        className="text-xs text-gray-500 mt-1"
                        data-oid="7tc_ts-"
                      >
                        You've completed "HDB Investment Masterclass". View your
                        certificate.
                      </p>
                      <p
                        className="text-xs text-gray-400 mt-1"
                        data-oid=":62vuh9"
                      >
                        Yesterday
                      </p>
                    </div>
                    <div className="px-4 py-3" data-oid="w29mg9b">
                      <p
                        className="text-sm font-medium text-gray-800"
                        data-oid="j3uh_p9"
                      >
                        Upcoming webinar
                      </p>
                      <p
                        className="text-xs text-gray-500 mt-1"
                        data-oid="cnmbk1-"
                      >
                        "Property Market Trends 2025" webinar starts in 2 days.
                      </p>
                      <p
                        className="text-xs text-gray-400 mt-1"
                        data-oid="k.n.3d6"
                      >
                        2 days ago
                      </p>
                    </div>
                  </div>
                  <div
                    className="px-4 py-2 border-t border-gray-100"
                    data-oid="vjpnkih"
                  >
                    <button
                      className="text-sm text-blue-600 hover:underline w-full text-center"
                      data-oid="7:n43t_"
                    >
                      View all notifications
                    </button>
                  </div>
                </div>
              )}
            </div>

            <div className="relative" data-oid="g.a6gzj">
              <button
                onClick={toggleDropdown}
                className="flex items-center space-x-2 focus:outline-none border border-gray-200 rounded-full pl-1 pr-2 py-1 hover:bg-gray-50"
                data-oid="j03mbth"
              >
                <div
                  className="w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center overflow-hidden"
                  data-oid=".afhd2u"
                >
                  <Image
                    src="/profile-placeholder.png"
                    alt="Profile"
                    width={32}
                    height={32}
                    className="object-cover"
                    data-oid="m6xd0bt"
                  />
                </div>
                <span className="font-medium text-sm" data-oid="02etvhu">
                  Melvin Lim
                </span>
                <ChevronDown
                  className={`h-4 w-4 text-gray-500 transition-transform ${isDropdownOpen ? "rotate-180" : ""}`}
                  data-oid=":279se7"
                />
              </button>

              {isDropdownOpen && (
                <div
                  className="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-lg py-2 z-50 border border-gray-100"
                  data-oid="i4jed9r"
                >
                  <div
                    className="px-4 py-3 border-b border-gray-100"
                    data-oid="sf3a05f"
                  >
                    <p
                      className="text-sm font-medium text-gray-900"
                      data-oid=".xwg8wv"
                    >
                      melvin.lim@example.com
                    </p>
                  </div>

                  <button
                    onClick={() => handleNavigation("/profile/account")}
                    className="w-full text-left block px-4 py-2 hover:bg-gray-50"
                    data-oid="n6kbbt1"
                  >
                    <div
                      className="flex items-center space-x-2"
                      data-oid="6flh:f5"
                    >
                      <User
                        className="h-5 w-5 text-gray-500"
                        data-oid="df3w.ae"
                      />

                      <span className="text-sm" data-oid="az.ood0">
                        Account Settings
                      </span>
                    </div>
                  </button>

                  <button
                    onClick={() =>
                      handleNavigation("/profile/purchase-history")
                    }
                    className="w-full text-left block px-4 py-2 hover:bg-gray-50"
                    data-oid="b6l.3t5"
                  >
                    <div
                      className="flex items-center space-x-2"
                      data-oid="jokxdd3"
                    >
                      <ShoppingBag
                        className="h-5 w-5 text-gray-500"
                        data-oid="l6aituj"
                      />

                      <span className="text-sm" data-oid="mgmaaih">
                        Purchase History
                      </span>
                    </div>
                  </button>

                  <button
                    onClick={() => handleNavigation("/profile/settings")}
                    className="w-full text-left block px-4 py-2 hover:bg-gray-50"
                    data-oid="0:6b:7v"
                  >
                    <div
                      className="flex items-center space-x-2"
                      data-oid="-1fw.19"
                    >
                      <Settings
                        className="h-5 w-5 text-gray-500"
                        data-oid="64el3lb"
                      />

                      <span className="text-sm" data-oid="an4cvex">
                        Settings
                      </span>
                    </div>
                  </button>

                  <button
                    onClick={() => handleNavigation("/profile/terms")}
                    className="w-full text-left block px-4 py-2 hover:bg-gray-50"
                    data-oid="7c3wj5k"
                  >
                    <div
                      className="flex items-center space-x-2"
                      data-oid="r8vgd5b"
                    >
                      <FileText
                        className="h-5 w-5 text-gray-500"
                        data-oid="sqte64b"
                      />

                      <span className="text-sm" data-oid="0-uzxz5">
                        Terms of Use
                      </span>
                    </div>
                  </button>

                  <div
                    className="border-t border-gray-100 mt-2 pt-2"
                    data-oid="-wok401"
                  >
                    <button
                      className="w-full text-left px-4 py-2 hover:bg-gray-50 text-red-600"
                      data-oid="1ld9fpd"
                    >
                      <div
                        className="flex items-center space-x-2"
                        data-oid="nyzvuj7"
                      >
                        <LogOut className="h-5 w-5" data-oid="1mrzco2" />
                        <span className="text-sm" data-oid="o9-ikw9">
                          Log out
                        </span>
                      </div>
                    </button>
                  </div>
                </div>
              )}
            </div>
          </div>
        </div>

        <div className="md:hidden flex items-center" data-oid="cn14j53">
          <CartIcon data-oid="0rfjsg0" />
          <button className="ml-4 text-gray-600" data-oid="jnt:kr4">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-6 w-6"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              data-oid="4flagnz"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M4 6h16M4 12h16M4 18h16"
                data-oid="jyv8qu9"
              />
            </svg>
          </button>
        </div>
      </div>
    </nav>
  );
}
