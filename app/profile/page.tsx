...
import { BookOpen, ChevronRight, Heart, Award, ShoppingBag, Calendar } from "lucide-react"
import {
  getEnrolledCourses,
  getCompletedCourses,
...
export default function ProfilePage() {
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">{greeting}, Melvin</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          >
            ...
            <div className="p-5">
              <div className="flex justify-between items-start">
                ...
                >
                  <BookOpen className={`h-4 w-4 ${hoveredCard === "enrolled" ? "text-white" : "text-blue-700"}`} />
                </div>
              </div>
              <p className="text-3xl font-bold text-blue-900 mt-3">12</p>
          ...
          >
            ...
            <div className="p-5">
              <div className="flex justify-between items-start">
                ...
                >
                  <Award className={`h-4 w-4 ${hoveredCard === "completed" ? "text-white" : "text-blue-700"}`} />
                </div>
              </div>
              <p className="text-3xl font-bold text-blue-900 mt-3">8</p>
              <p className="text-sm text-green-500 flex items-center mt-1">
                <svg
                  width="14"
                  height="14"
          ...
          >
            ...
            <div className="p-5">
              <div className="flex justify-between items-start">
                ...
                >
                  <Heart className={`h-4 w-4 ${hoveredCard === "wishlist" ? "text-white" : "text-blue-700"}`} />
                </div>
              </div>
              <p className="text-3xl font-bold text-blue-900 mt-3">7</p>
              <p className="text-sm text-green-500 flex items-center mt-1">
                <svg
                  width="14"
                  height="14"
        ...
        {/* Upcoming Events */}
        <Link href="/profile/events" className="block">
          <div
            className="bg-white rounded-lg shadow-sm relative overflow-hidden cursor-pointer"
          ...
          >
            ...
            <div className="p-5">
              <div className="flex justify-between items-start">
                <h3 className="text-gray-700 font-medium">Upcoming Events</h3>
                <div
                  className={`p-2 rounded-full transition-colors duration-200 ${
                    hoveredCard === "events" ? "bg-blue-700" : "bg-blue-100"
                  }`}
                >
                  <Calendar className={`h-4 w-4 ${hoveredCard === "events" ? "text-white" : "text-blue-700"}`} />
                </div>
              </div>
              <p className="text-3xl font-bold text-blue-900 mt-3">{upcomingEvents.length}</p>
              <p className="text-sm text-green-500 flex items-center mt-1">
                <svg
                  width="14"
                  height="14"
              ...
              {enrolledCourses.map((course) => (
                ...
                >
                  ...
                  <div className="flex-grow">
                    ...
                    <p className="text-xs text-gray-500 mt-1">{course.totalHours} hours total</p>
                    <div className="mt-2 w-full bg-gray-200 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full ${
                          course.progress > 70 ? "bg-green-600" : course.progress > 30 ? "bg-green-500" : "bg-green-400"
                        }`}
                    ...
                    <div className="mt-2 flex justify-between items-center">
                      <span className="text-xs text-gray-500">
                        {course.progress === 0 ? "Not started yet" : `Last Accessed: ${course.lastAccessed}`}
                      </span>
                      <span className="text-xs text-green-600 font-medium">
                        {course.progress === 0 ? "Ready to start" : `${course.progress}% completed`}
                      </span>
                    </div>
          ...
          >
            <div className="flex justify-between items-center mb-4">
              <div>
                ...
                <p className="text-sm text-gray-500">Your recent learning activity</p>
              </div>
              <Link
              ...
              {recentActivity.map((activity) => (
                <div key={activity.id} className="border-b border-gray-100 last:border-b-0 pb-4 last:pb-0">
                  <div className="flex items-start gap-3">
                    ...
                    >
                      {activity.type === "completed" && <Award className={`h-4 w-4 text-green-600`} />}
                    ...
                    <div>
                      ...
                      {activity.course && <p className="text-sm text-gray-600 mt-1">{activity.course}</p>}
                      {activity.instructor && (
                        <p className="text-sm text-gray-600 mt-1">Course by {activity.instructor}</p>
                      )}
                      {activity.price && <p className="text-sm text-gray-600 mt-1">{activity.price} Premium Course</p>}
                      {activity.xp && <p className="text-sm text-green-600 mt-1">+{activity.xp} XP earned</p>}
          ...
          >
            <div className="flex justify-between items-center mb-4">
              <div>
                ...
                <p className="text-sm text-gray-500">Based on your interests and learning history</p>
              </div>
              <Link
                href="/courses"
                className="text-blue-600 text-sm font-medium flex items-center gap-1 hover:underline"
                ...
                >
                  ...
                  <div className="flex-grow">
                    ...
                    <div className="flex items-center justify-between mt-1">
                      ...
                      <span className="text-xs text-gray-500">{course.totalHours} hours</span>
                    </div>
                    <Link
                      href={`/courses/${course.slug}`}
