...
export default function CourseCard({ course, delay = 0, size = "normal" }: CourseCardProps) {
    ...
    return (
      <Link href={courseLink} className="block h-full">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow flex flex-col h-full p-4 items-center">
            {/* Image */}
            <div className="w-full flex justify-center">
              <div className="rounded-xl overflow-hidden w-full" style={{ height: 120, maxWidth: 220 }}>
                <Image
                  src={course.image || "/placeholder.svg?height=120&width=220&query=real+estate+course"}
                  alt={course.title}
                  width={220}
                  height={120}
            ...
            <div className="w-full mb-2">
              <span className="block font-semibold text-gray-900 text-sm line-clamp-2">{course.title}</span>
            </div>
            ...
            <div className="flex items-center w-full mb-2 text-xs text-gray-500">
              {course.rating && course.rating > 0 ? (
                <>
                  ...
                  <span className="ml-1">{course.reviewCount ? `${course.reviewCount} reviews` : "No reviews"}</span>
                </>
              ) : (
            ...
            <div className="flex items-center justify-between w-full mt-auto pt-2">
              <span className={`font-semibold ${isFree ? "text-green-600" : "text-gray-900"}`}>{displayPrice}</span>
              {inCart ? (
                <Button disabled className="bg-gray-200 text-gray-500 font-medium rounded px-3 py-1 text-xs cursor-not-allowed">In Cart</Button>
              ) : isFree ? (
                <Button
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
    >
      <div className={`bg-white rounded-lg overflow-hidden h-full flex flex-col border border-gray-200 shadow-sm hover:shadow-md transition-shadow ${cardPadding}`}>
              ...
              src={
                course.title === "Rising Stars or Hidden Gems?"
                  ...
                  : course.title === "Freehold Property Investment Strategy"
                    ...
                    : course.title === "The Ultimate Showdown: Cluster Houses vs. Condos vs. Landed Properties"
                      ...
                      : course.title === "Landed Property Investment Strategies"
                        ...
                        : course.title === "The Art of Real Estate Investment"
                          ...
                          : course.title === "Exit with Confidence"
                            ...
                            : course.title === "Condo Investment Workshop: Building a Profitable Property Portfolio with Confidence"
                              ...
                              : course.title === "Landed Buyer Investing"
                               ? "/landed-buyer-investing.jpg"
                                : course.title === "Selling Your Property Effectively as a DIY Property Seller"
                                 ? "/selling-your-property-effectively-as-a-diy-property-seller.jpg"
                                 : course.title === "Property Financing Strategy Mastery"
                                   ? "/property-financing-strategy-mastery.jpg"
                                   :course.title === "Are Landed Properties Overpriced in 2024? Should We Wait for 2025 to Enter or Is Now the Best Time?"
                                    ? "/are-landed-properties-overpriced-in-2024-should-we-wait-for-2025-to-enter-or-is-now-the-best-time.jpg"
                                    : course.image || "/placeholder.svg?height=200&width=400&query=real+estate+course"
              }
              alt={course.title}
              fill
        ...
        <div className={`flex-1 flex flex-col ${cardPadding}`}>
          ...
          {course.categories && course.categories.length > 0 && (
            <div className="flex flex-wrap gap-1 mb-2">
              {course.categories.map((category, index) => (
                <span key={index} className="text-xs font-medium px-2 py-0.5 bg-[#E6EDF8] text-[#123B79] rounded-full">
                  {category}
                </span>
              ))}
            </div>
          ...
          <Link href={courseLink} className="block">
            <h3 className={`font-medium text-gray-900 mb-2 line-clamp-2 group-hover:text-[#123B79] ${titleSize}`}>{course.title}</h3>
            <p className={`text-gray-500 mb-2 ${fontSize}`}>By {course.instructor}</p>
          </Link>
          ...
          <div className={`flex items-center text-gray-500 mb-4 ${fontSize}`}>
            {course.rating && course.rating > 0 ? (
              <span className="flex items-center">
                ...
                {course.rating.toFixed(1)}
                {course.reviewCount && <span className="ml-1">({course.reviewCount})</span>}
              </span>
            ) : (
              <span>No Reviews Yet</span>
            )}
          ...
          <div className="mt-auto pt-3">
            ...
            <div className="flex items-center justify-between">
              {/* Price display */}
              <div className={`font-medium ${fontSize} ${isFree ? "text-green-600" : "text-gray-900"}`}>{displayPrice}</div>
