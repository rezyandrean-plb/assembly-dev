...
export default function NetworkBackground({ scrollY, scrollSpeed, windowHeight, opacity = 1 }: NetworkBackgroundProps) {
  ...
  useEffect(() => {
    if (!scene || !camera || !nodes || !lines || !particles || !windowHeight) return
    ...
    nodes.forEach((node, index) => {
      ...
      node.scale.setScalar(baseScale + (targetScale - baseScale) * scrollProgress)
    })
    ...
    if (particles) {
      ...
      const rotationFactor = Math.min(scrollSpeed * 0.0001 * scrubbingRate * 0.5, 0.0025)
    ...
    render()
  }, [scrollY, scrollSpeed, windowHeight, scene, camera, nodes, lines, particles, setNetworkState])
